    ...
    <main className="min-h-screen bg-brand-black selection:bg-brand-yellow selection:text-brand-black">
      ...
      <div className="pt-40 pb-20 px-6 md:px-16 bg-pixel-grid relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-brand-yellow opacity-5 blur-[120px] rounded-full" />
          <div className="absolute bottom-1/4 left-1/4 w-64 h-64 bg-brand-yellow opacity-5 blur-[120px] rounded-full" />
        </div>
        ...
        <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          ...
          >
            <h1 className="text-brand-white font-press-start text-3xl leading-tight mb-8">
              Start reviewing code <span className="text-brand-yellow text-4xl">10x faster.</span>
            </h1>
            ...
            <ul className="space-y-6 mb-12">
              {[
                ...
                <li key={feature} className="flex items-center gap-4 text-gray-300 font-mono text-sm">
            ...
            <div className="p-6 bg-[#0C0C0C] border border-gray-800 rounded-xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-2 opacity-10 group-hover:opacity-30 transition-opacity">
                <Image src="/icon.png" alt="" width={60} height={60} />
              </div>
              <p className="text-gray-400 italic text-sm mb-4 relative z-10">
                &quot;GitRabbit has completely changed how our team handles PRs. The AI suggestions are spookily accurate.&quot;
              </p>
              <div className="flex items-center gap-3 relative z-10">
                <div className="w-8 h-8 bg-brand-yellow/20 rounded-full flex items-center justify-center border border-brand-yellow/30 font-press-start text-[8px] text-brand-yellow">R</div>
                <div>
                  <p className="text-brand-white text-xs font-bold">Alex Rivera</p>
                  <p className="text-gray-600 text-[10px] uppercase">Engineering Manager @ Vercel</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="bg-[#0C0C0C] border border-gray-800 p-8 rounded-2xl shadow-2xl relative">
              ...
              <form className="space-y-4">
                <div>
                  <label className="block text-[9px] font-press-start text-brand-yellow uppercase mb-2">Full Name</label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600" />
                    <input type="text" placeholder="John Doe" className="w-full bg-brand-dark border border-gray-800 rounded-lg py-2.5 px-10 text-brand-white focus:border-brand-yellow focus:outline-none transition-colors font-mono text-sm" />
                  </div>
                </div>

                <div>
                  <label className="block text-[9px] font-press-start text-brand-yellow uppercase mb-2">Email</label>
                  <div className="relative">
